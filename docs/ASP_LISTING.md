...
Status: Conviction ASP `#7034` is listed with the final two-service catalog submitted on 2026-07-22. The update transaction is `0x861be48333c6ec751ea615b4122816509cfe0679b92b555ffa5815824055cf60`. Production v0.4.19 keeps the submitted names, prices, endpoints, and outcome promise while making the paid machine contract stricter: structured unsupported buyers are rejected before x402 verification and receive no service or funding instructions, venue-wallet stop metadata is limited to real venue-wallet blockers, OPEN remains ready-deposit-wallet-only, official-plugin dry-run agreement is required before issuance, a free payment-bound card refresh window is available, and both current and legacy buyer-controlled Polymarket deposit wallets are supported.
