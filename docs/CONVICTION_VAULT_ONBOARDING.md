...
The separate onboarding path uses Polymarket's current official relayer route: a buyer-controlled browser EVM signer and a server-held dedicated Relayer API key, with Builder credentials retained only as a fallback authentication mode. It does not bypass an Agentic Wallet policy and it never takes custody of a buyer key or CLOB credential. After setup, the browser adapter keeps the buyer's ephemeral CLOB authentication local and executes only after the paid card is displayed and the buyer gives separate trade consent. The existing agent/plugin paid route remains unchanged.
...
The server rejects noncanonical JSON, extra fields, another buyer, another factory, another method or path, a deadline beyond five minutes, a substituted call, and a batch whose EIP-712 signature does not recover the authenticated buyer. Relayer or fallback Builder credentials are used only after validation and never leave the server.
...
1. Create a Polymarket Relayer API key at `polymarket.com → Settings → Relayer API keys`. Builder credentials may be configured only as the documented fallback.
2. Store the Relayer API key and its authorized address only in encrypted server environment variables. Any fallback Builder key, secret, and passphrase receive the same treatment. They must never enter browser JavaScript, Git, client logs, x402 responses, or chat.
