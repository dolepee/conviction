...
Buyer readiness is a deterministic classifier over buyer-supplied local capability, wallet, balance, venue, mode, and approval state. It never reads credentials or signs, funds, approves, pays, or trades. A missing deposit wallet, balance, selected mode, V2 approval, or regional check is recoverable `BUYER_SETUP_REQUIRED`/`REGION_CHECK_REQUIRED`; each response provides both the next action and the complete remaining setup plan. `UNSUPPORTED_EXECUTION_RUNTIME` means the runtime genuinely lacks wallet, x402-payment, or Polymarket-trading capability. The published X Layer payee lets the buyer refuse `payer == payee` before preview or payment. The one-time relayer-paid V2 deposit-wallet setup requires no POL and uses two maximum pUSD allowances plus three blanket CTF operator approvals; every readiness response discloses their reusable and currently non-revocable scope. Funding instructions distinguish the X Layer payer from the separate Polygon deposit wallet and forbid sending pUSD to the Polygon EOA. Conviction never bypasses buyer or organization policy.
