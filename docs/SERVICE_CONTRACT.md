...
`cancel-tp` requires the distinct exact phrase `confirm cancel take profit`. It invokes only `polymarket-plugin cancel --order-id <pinned-order-id>`, never a market-wide or all-order cancel, and then re-fetches the exact order. A fill/cancel race returns the matched quantity and still requires chain proof. A missing or indeterminate lookup remains `UNKNOWN` with reconciliation required; a plugin cancel acknowledgement alone is not proof of cancellation. If cancellation retains the global execution lock, read-only-external `reconcile-tp` releases only the owner-verified exact lock after a terminal zero-fill status or a terminal finalized fill proof. Provisional, active, partial-live, or unknown state keeps the lock.
