...
Buyer readiness is a deterministic classifier over buyer-supplied local capability, wallet, balance, venue, mode, and approval state. It never reads credentials or signs, funds, approves, pays, or trades. A missing deposit wallet, balance, selected mode, V2 approval, or regional check is recoverable `BUYER_SETUP_REQUIRED`/`REGION_CHECK_REQUIRED`; `UNSUPPORTED_EXECUTION_RUNTIME` means the runtime genuinely lacks wallet, x402-payment, or Polymarket-trading capability. The published X Layer payee lets the buyer refuse `payer == payee` before preview or payment. The one-time official V2 deposit-wallet setup uses two reusable pUSD allowances and three reusable CTF operator approvals; Conviction discloses this and never bypasses buyer or organization policy.
