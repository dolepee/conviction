...
3. Only after reviewing those bounds does the buyer bind their configured Polygon trading wallet and receive a five-minute wallet-bound position-card preview plus a secure dry-run prompt. Deposit-wallet buyers use their configured deposit wallet. First-time buyers may instead use finite-approval EOA mode for OPEN only.
...
`GET /api/readiness` publishes the free readiness contract and `POST /api/readiness` classifies a buyer-local wallet snapshot before preview or payment. A missing balance, selected trading mode, approval, regional check, or supported runtime returns `BUYER_SETUP_REQUIRED` or `REGION_CHECK_REQUIRED` with one exact next action plus the complete remaining setup plan; `UNSUPPORTED_EXECUTION_RUNTIME` is reserved for a runtime that genuinely lacks wallet, x402-payment, or Polymarket-trading capability. Every result carries the approval disclosure and exact funding destinations: USD₮0 goes to the X Layer payer, and pUSD goes to the selected Polygon trading wallet. Deposit-wallet mode requires no POL gas. Finite-approval EOA mode requires direct pUSD on the Polygon EOA plus about `0.01 POL` for the one approval transaction. The classifier also publishes Conviction's X Layer payee and rejects `payer == payee` as `SELF_PAYMENT_FORBIDDEN`.

For first-time OPEN, Conviction now supports a policy-compatible EOA path on standard non-neg-risk V2 markets: after a successful free preview, the buyer approves exactly the preview's signed maximum debit to the standard V2 pUSD exchange, never `MAX_UINT`, and never `setApprovalForAll`. The runner reads the allowance back on Polygon and requires it to be at least the order principal and no more than the approved maximum before it will allow payment or trade. It then invokes the official Polymarket plugin with `--mode eoa` and without `--approve`, so the plugin skips its unlimited-approval branch.

The venue-managed deposit-wallet setup remains supported for ready buyers and remains required for CLOSE and TAKE_PROFIT because selling outcome tokens uses ERC-1155 operator approval. That one-time relayer batch creates two reusable pUSD allowances and three reusable CTF operator approvals. Conviction cannot weaken or bypass the buyer runtime's wallet policy; it gives policy-constrained buyers a bounded OPEN route instead.
...
For OPEN, the native prerequisites are an active OKX Agentic Wallet, official `polymarket-plugin` v0.7.0, X Layer USD₮0 for the service fee, regional eligibility, and either persisted `deposit-wallet` mode or finite-approval EOA mode. EOA mode requires direct pUSD on the Polygon EOA and enough POL for the finite pUSD approval; it supports OPEN only. CLOSE still needs persisted `deposit-wallet` mode, the selected outcome shares, and the standard V2 outcome-token approval. The discovery document publishes the official Plugin Store artifact digests for Apple Silicon and hosted Linux x64; the release gate downloads and verifies the official Linux artifact. TAKE_PROFIT remains on the fallback below because official v0.7.0 transports V2 GTD expiration in a location the live CLOB rejects.
...
Cold buyer agents discover execution before payment through `GET /api/executor` or the `service-desc` link on either x402 challenge. Every new paid card binds the same executor-release hash into its signed intent and returns an action-specific structured next step. OPEN prefers native OKX and publishes both deposit-wallet and finite-approval EOA modes with `convictionInstallRequired:false`; CLOSE prefers native deposit-wallet execution; TAKE_PROFIT explicitly selects the pinned fallback. All paths remain buyer-local and require separate trade confirmation.
...
New Polymarket deposit-wallet setup currently grants max pUSD allowances and blanket ERC-1155 approvals to official Polymarket exchange contracts. Conviction discloses this before execution and recommends a dedicated low-balance wallet. For first-time OPEN on standard non-neg-risk V2 markets, the finite EOA path avoids those broad approvals by approving only the signed pUSD debit ceiling to the standard exchange; it does not cover CLOSE or TAKE_PROFIT. The current official plugin has no deposit-wallet revoke command.
...
- Production runtime: v0.4.14 adds finite-approval EOA OPEN preparation for first-time policy-constrained buyers while preserving v0.4.13 readiness, write-once live evidence, v0.4.10 market lookup fix, v0.4.9 signed-proof runtime, issuer-signed public proofs, YES/NO CTF label binding, and immutable executor v6
