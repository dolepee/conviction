...
- Production runtime: v0.4.21 preserves the agent/plugin execution path and adds a browser-native OPEN journey for already-ready buyer-controlled Polymarket deposit wallets: free preview, exact buyer-local x402 payment, issuer-signed v4 card, separate trade consent, official browser SDK execution, and the existing Polygon proof. It rechecks wallet ownership, deployed code, all five venue approvals, and pUSD balance before payment. The browser path is release-gated but remains explicitly acceptance-pending until a fresh controlled live run completes
