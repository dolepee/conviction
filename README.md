...
`GET /api/readiness` publishes the free readiness contract and `POST /api/readiness` classifies a buyer-local wallet snapshot before preview or payment. Conviction charges OPEN only when the selected Polygon trading address is an already-deployed Polymarket deposit-wallet contract, V2 setup is ready, regional access passed, and both chain balances are sufficient. A missing or unfinished venue wallet returns `BUYER_SETUP_REQUIRED`, `paymentAllowed:false`, and `nextAction:USE_READY_DEPOSIT_WALLET_OR_STOP`; it receives no service or funding instructions. It also returns a separate browser-only handoff, which must first verify its own activation and Builder authorization before any funding. A direct EOA is never presented as a recoverable execution route because a live external run proved Polymarket V2 rejects that maker. The classifier publishes Conviction's X Layer payee and rejects `payer == payee` as `SELF_PAYMENT_FORBIDDEN`.

Before payment, the buyer must provide the official plugin's successful dry run. Conviction compares its wallet, token, outcome, exchange, collateral, price, shares, principal, fee rate, and FAK order type with the server-compiled preview. A mismatch refuses the paid replay. The venue-managed deposit-wallet setup remains outside Conviction: it is relayer-paid but creates two reusable pUSD allowances and three reusable CTF operator approvals. Conviction discloses that scope, never asks a buyer runtime to bypass its wallet policy, and does not turn first-time setup into a funding loop.
...
Cold buyer agents discover execution before payment through `GET /api/executor` or the `service-desc` link on either x402 challenge. The challenge also declares the exact paid replay schema. Every new paid card binds the same executor-release hash into its signed intent and returns an action-specific structured next step. OPEN and CLOSE use native ready deposit-wallet execution; TAKE_PROFIT explicitly selects the pinned fallback. A verified OPEN payment may refresh an expired card for 30 minutes through `POST /api/refresh` without a second payment. All paths remain buyer-local and require separate trade confirmation.
...
- Production runtime: v0.4.24 preserves the agent/plugin execution path and adds a browser-native OPEN journey for already-ready buyer-controlled Polymarket deposit wallets: free preview, exact buyer-local x402 payment, issuer-signed v4 card, separate trade consent, official browser SDK execution, and the existing Polygon proof. It rechecks wallet ownership, deployed code, all five venue approvals, and pUSD balance before payment. New-buyer Deposit Wallet creation remains Builder-authenticated; the browser route verifies that Builder authorization through a read-only relayer check before it presents a deployment consent. The public setup status also checks that Builder authorization before it advertises an actionable route. An optional Relayer API key is accepted only for its exact associated account. The browser path is release-gated but remains explicitly acceptance-pending until a fresh controlled live run completes
...
- OKX.AI ASP: Conviction `#7034` is listed with its two-service catalog (`0.05 USDT` OPEN and `0.10 USDT` Position Manager). One independent buyer paid OPEN; the trade was rejected by Polymarket as `maker address not allowed`, so it is recorded as an external sale and failed journey—not an execution proof
