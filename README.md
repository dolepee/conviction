...
`GET /api/readiness` publishes the free readiness contract and `POST /api/readiness` classifies a buyer-local wallet snapshot before preview or payment. A missing deposit wallet, balance, trading mode, approval, or regional check returns `BUYER_SETUP_REQUIRED` or `REGION_CHECK_REQUIRED` with one exact next action plus the complete remaining setup plan; `UNSUPPORTED_EXECUTION_RUNTIME` is reserved for a runtime that genuinely lacks wallet, x402-payment, or Polymarket-trading capability. Every result carries the approval disclosure and exact funding destinations: USD₮0 goes to the X Layer payer, pUSD goes only to the separate Polygon deposit wallet, and deposit-wallet mode requires no POL gas. The classifier also publishes Conviction's X Layer payee and rejects `payer == payee` as `SELF_PAYMENT_FORBIDDEN`.
...
- Production runtime: v0.4.13 expands Buyer Readiness with a complete setup plan, unavoidable approval disclosure, exact funding destinations, labeled venue minimums, and commit-bound write-once live evidence that rechecks clean source before launch and report write while preserving the v0.4.10 market lookup fix, v0.4.9 signed-proof runtime, issuer-signed public proofs, YES/NO CTF label binding, and immutable executor v6
