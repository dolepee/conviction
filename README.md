...
`GET /api/readiness` publishes the free readiness contract and `POST /api/readiness` classifies a buyer-local wallet snapshot before preview or payment. Conviction charges OPEN only when the selected Polygon trading address is an already-deployed Polymarket deposit-wallet contract, V2 setup is ready, regional access passed, and both chain balances are sufficient. Missing setup returns `BUYER_SETUP_REQUIRED` with `paymentAllowed:false` and `nextAction:USE_READY_DEPOSIT_WALLET`; a direct EOA is never presented as a recoverable execution route because a live external run proved Polymarket V2 rejects that maker. The classifier publishes Conviction's X Layer payee and rejects `payer == payee` as `SELF_PAYMENT_FORBIDDEN`.

Before payment, the buyer must provide the official plugin's successful dry run. Conviction compares its wallet, token, outcome, exchange, collateral, price, shares, principal, fee rate, and FAK order type with the server-compiled preview. A mismatch refuses the paid replay. The venue-managed deposit-wallet setup remains an out-of-band prerequisite: it is relayer-paid but creates two reusable pUSD allowances and three reusable CTF operator approvals. Conviction discloses that scope and cannot bypass the buyer runtime's wallet policy.
...
- Production runtime: v0.4.17 declares exact A2MCP replay inputs, refuses structured unsupported makers before x402 verification, rate-limits the no-payment Polygon identity preflight, keeps bare marketplace discovery x402-compatible, requires exact official-plugin dry-run agreement, permits a payment-bound 30-minute OPEN card refresh without charging again, and supports both current and legacy buyer-controlled Polymarket deposit wallets
