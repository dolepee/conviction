...
- Production runtime: v0.4.19 declares exact A2MCP replay inputs, refuses structured unsupported makers before x402 verification, returns an explicit no-payment stop rather than fresh-wallet setup or funding instructions, attaches venue-wallet stop metadata only to an actual venue-wallet blocker, rate-limits the no-payment Polygon identity preflight, keeps bare marketplace discovery x402-compatible, requires exact official-plugin dry-run agreement, permits a payment-bound 30-minute OPEN card refresh without charging again, and supports both current and legacy buyer-controlled Polymarket deposit wallets
