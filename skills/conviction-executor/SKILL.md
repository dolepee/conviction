...
For `TAKE_PROFIT`, the immediate verification target is the exact authenticated CLOB order and `ARMED` passport. For later `tp-status`, fetch the exact pinned order and all associated authenticated trades, attribute maker/taker contributions without ambiguity, then independently rederive every unique Polygon receipt and aggregate partial/full fills against the signed wallet, token, share cap, target, gross, fee, and net bounds. Status is read-only and needs no payment. Never call a zero-match or indeterminate order a fill. Missing/incomplete order, trade, pagination, or chain data stays unresolved.
