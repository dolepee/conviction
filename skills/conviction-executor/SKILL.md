...
For OPEN or CLOSE, prefer the paid card's signed `native-okx-agentic-wallet` next step when the buyer agent already has OKX Agentic Wallet, the official OKX Plugin Store `polymarket-plugin` v0.7.0, and an already-ready buyer-controlled Polymarket deposit wallet. That path requires no Conviction installation or repository checkout: invoke the exact `executionCard.argv` through the official tool after separate trade confirmation. A direct EOA is not a supported OPEN fallback: the live V2 venue rejected it even after finite approval, so Conviction must refuse it before payment. Never type or ask the user to type the command. Do not use official v0.7.0 for TAKE_PROFIT; its V2 GTD transport is not accepted by the live CLOB, so the paid card must select the corrected fallback.
...
Call `POST https://conviction-bay.vercel.app/api/readiness` with the buyer-local capability, wallet, balance, venue, mode, and approval snapshot before the market preview. Treat its result as guidance, then independently recheck every field locally before payment and execution. Proceed only when `paymentAllowed:true`. A `BUYER_SETUP_REQUIRED` result whose next action is `USE_READY_DEPOSIT_WALLET_OR_STOP` is a no-payment stop, not an in-session onboarding path: do not preview, fund, approve, pay, or trade. If it includes `browserSetupHandoff`, present it as an optional separate browser route, but never open, automate, or imply approval of that route from this agent session. It may resume only after the browser route independently verifies Builder authorization and the buyer has a ready buyer-controlled deposit wallet. `REGION_CHECK_REQUIRED` is a read-only check; only `UNSUPPORTED_EXECUTION_RUNTIME` means the runtime genuinely lacks wallet, x402-payment, or Polymarket-trading capability. `SELF_PAYMENT_FORBIDDEN` and `REGION_RESTRICTED` are final for the active session.
...
3. A buyer-controlled deposit wallet already exists, is the persisted active trading mode, and exactly matches the request wallet.
...
Use only the validated `executionCard.argv`, passed as separate argument values. Append only `--dry-run` for the preview pass; any other argv mutation stops the flow.
...
Never add `--mode deposit-wallet`, `--mode eoa`, `--approve`, `--confirm`, `--round-up`, `--autotrade-job`, a retry, or rerouting flags. Preserve `--post-only` only when it is already present in a helper-validated TAKE_PROFIT vector; never append it independently. The plugin selects the already-persisted deposit wallet; independently verify that wallet before dry run and again immediately before live execution.
...
- active Polygon trading wallet and selected `deposit-wallet` mode;
